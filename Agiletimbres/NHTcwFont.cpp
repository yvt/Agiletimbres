...
NHTcwFont::NHTcwFont(unsigned char *_fontData, unsigned int *_indexMap, unsigned char *_widthMap){
	fontData=_fontData;
	indexMap=_indexMap;
	widthMap=_widthMap;
	topMargin=0;
...
void NHTcwFont::render(twDC *dc, twColor color, const twPoint& pos, const std::wstring& str) const{
	if(!dc)
		return;
	...
	if(surf->format->BitsPerPixel!=16){
		throw twException(L"NHTcwFont supports only 16-bit color depth.");
	}
...
twSize NHTcwFont::measure(const std::wstring& str) const{
	wchar_t br=0;
...
int NHTcwFont::widthForScaledGlyph(int baseWidth, int height) const{
	return baseWidth*height/actualLineHeight;
}

/* scaled version. */
void NHTcwFont::render
...
void NHTcwFont::drawGlyphScaled(twSDLDC *sdlDC, uint16_t nativeColor,
...
void NHTcwFont::drawGlyphScaledSpan(uint16_t *destSpan,
...
twSize NHTcwFont::measure(const std::wstring& str, int size) const{
	wchar_t br=0;
	twPoint pt(0,size);
	int mw=0;
